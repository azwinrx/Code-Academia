...
import { getMateriDetailWithProgress, addProgress } from "../helper/supabaseMateri";
...
export default function MateriDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError(null);
      try {
        const { materi, subMateriList, completedIds } = await getMateriDetailWithProgress(slug, user.id);
        if (!materi) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen bg-slate-900"><p className="text-white text-2xl">Loading...</p></div>;
  if (error) return <div className="flex items-center justify-center h-screen bg-slate-900"><p className="text-red-500 text-2xl">{error}</p></div>;

  const progress = subMateriList.length > 0 ? Math.round((completedIds.size / subMateriList.length) * 100) : 0;
