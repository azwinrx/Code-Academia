  ...
  return (
    <main className="flex-1 p-6 md:p-8 overflow-y-auto">
      <section id="header-sambutan">
        <h2 className="text-3xl font-bold text-black">Good morning, {user?.user_metadata.full_name || 'Explorer'}</h2>
        <p className="mt-1 text-slate-400">You are on the right track to become an expert!</p>
      </section>
      ...
      <div style={{backgroundColor: pastelColors[3]}} className="mt-6 text-slate-800 p-6 rounded-xl shadow-lg flex flex-col md:flex-row items-center gap-6">
        <div className="flex-1">
          ...
          <p className="text-sm mt-1">Here is a summary of your overall learning progress.</p>
          ...
          <div className="mt-4 flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold">{courses.filter(c => c.progress === 100).length} Courses Completed</p>
              <p className="text-sm">out of {courses.length} total courses</p>
            </div>
          </div>
        </div>
      ...
      {courseToContinue && (
        <section className="mt-8">
          <h3 className="text-2xl font-bold text-black">Continue Learning</h3>
          <div style={{backgroundColor: pastelColors[5]}} className="mt-4 p-4 rounded-xl shadow-lg flex flex-col md:flex-row items-center gap-6 text-slate-800">
            <div className="w-full">
              <p className="text-sm">Status: {courseToContinue.status}</p>
              ...
              <button onClick={() => handleCourseClick(courseToContinue.slug)} className="mt-6 w-full md:w-auto bg-white/50 font-semibold py-2 px-6 rounded-lg hover:bg-white/80 transition-colors">
                Continue Learning
              </button>
            </div>
          </div>
      ...
      <section id="kursus-saya" className="mt-8">
        <h3 className="text-2xl font-bold text-black">My Courses</h3>
